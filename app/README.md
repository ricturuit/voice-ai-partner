...
3. **無音検知の合図音**: 無音カウントダウンが始まって1秒経過した最初の
   タイミング(=実際に間が空いたと判断できた瞬間)で、控えめな「ポン」音を
   1回だけ再生する(`assets/sounds/silence_cue.wav`、音量0.18、専用の
   `AudioPlayer`インスタンスで再生し、返答音声の再生ロジックとは独立)。
   話し続けている間はカウントダウンがリセットされ続けるため鳴らない。
   既存の著作権付き効果音(iOS標準の通知音など)は使わず、短い正弦波トーン
   (約220ms、880Hz、指数減衰エンベロープ)をこちらで新規に合成した
   ものを同梱している。
