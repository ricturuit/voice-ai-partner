...
- マイクボタンをタップすると音声入力(STT)を開始。認識結果はテキスト入力欄に
  リアルタイムで反映される。**発話が終わった(無音区間を検知した)ことを
  ブラウザの音声認識エンジン自身が判定すると、自動的に送信される**
  (会話として連続でやり取りできるようにするため。手動での送信ボタン操作は
  不要)。認識中は入力欄上に赤いインジケーター(「音声を認識しています…」)を表示
...
- 音声認識の結果コールバック(`onResult`)で`result.finalResult`が`true`に
  なったタイミング(=ブラウザの音声認識エンジンが無音区間を検知して発話終了
  と判断したタイミング)で自動的に送信処理(`_handleSend`)を呼び出す。連続で
  会話するために手動での送信ボタン操作を不要にしている。送信ボタン自体は
  常に表示・利用可能なままで、テキスト入力からの送信経路は変えていない
