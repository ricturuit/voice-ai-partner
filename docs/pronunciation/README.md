...
└── scripts/
    ...
    └── build.js              validate → generated/ の再生成
                               (+ infra/lambda/conversation/pronunciation-lookup.json への書き出し)
```

`generated/lookup.json` と同じ内容が
`infra/lambda/conversation/pronunciation-lookup.json` にもコピーされる。
このファイルは(`generated/` と異なり)git管理されており、
`infra/lambda/conversation/index.js` の `toTtsText()` が実際に読み込む
実体(=名取の発音を決める本番データ)である(「Lambdaとの接続」参照)。
...
npm run build       # validate を内部で実行したうえで generated/ と
                     # infra/lambda/conversation/pronunciation-lookup.json を再生成
```

`generated/` はビルド成果物なので git 管理していない(`.gitignore` 参照)。
一方 `infra/lambda/conversation/pronunciation-lookup.json` は
`build` の実行結果として上書きされる**が、git管理されている**
(Lambdaの実行時にそのままrequireされる本番データのため)。

## 更新フロー

```
taxonomy → dictionary → validate → build → generated + infra/lambda/conversation/pronunciation-lookup.json → cdk deploy
```

1. 必要なら `taxonomy/categories.yaml` / `taxonomy/tags.yaml` を更新
2. `dictionary/<category>.yaml` に用語を追加・編集
3. `npm run validate` で整合性を確認
4. `npm run build` で `generated/` と
   `infra/lambda/conversation/pronunciation-lookup.json` を再生成
5. `generated/*.json` を見て意図した読みになっているか確認
6. `infra/lambda/conversation/pronunciation-lookup.json` の差分を
   コミットし、`infra/` で `npx cdk deploy InfraStack` を実行して
   実際のLambdaに反映する(4で再生成しただけではデプロイ済みの
   Lambdaには反映されない)
...
ダミーデータを実用データへ置き換え、続けて発音表記ルールの詳細化に
合わせて既存データを修正・拡充した(詳細は `taxonomy/decision_log.md`
の該当エントリを参照)。

### Lambdaとの接続

`index.js` 側は `generated/lookup.json` と同内容の
`infra/lambda/conversation/pronunciation-lookup.json`(git管理下)を
`fs.readFileSync` + `JSON.parse` でコールドスタート時に1回読み込み、
`toTtsText()` がそれを使って置換する構成に切り替え済み。旧来の
ハードコードされた `TTS_PRONUNCIATION_OVERRIDES`(AWS/S3/EC2/CDK/API/
SDK/URL/HTTP/HTTPS/JSON/CLI/せんせえ の12件)は削除した(全件が本辞書に
移行済みのため)。

この接続に合わせて `toTtsText()` 自体も書き直した。

- 旧実装は非英数字termを正規表現の特殊文字としてエスケープせずにその
  まま `RegExp` へ渡していた(`.NET`・`CI/CD`・`C++` 等で誤動作しうる
  懸念がすでに `decision_log.md` に記録されていた)。新実装は全termを
  `RegExp` 用にエスケープしてから使う。
- 境界判定を `\b`(ASCII文字のみ認識、日本語には効かない)から、
  「直前直後がASCII英数字でないこと」を要求する先読み・後読み
  (`(?<![A-Za-z0-9])`...`(?![A-Za-z0-9])`)に変更した。`\b` と違い
  日本語文字列にも一律に適用でき、`AI` が `AIRPORT` の内部で誤爆しない
  ことをテスト済み。
- 複合語(`CI/CD` 等)がその部分文字列(`CI`、`CD`)によって先に
  置換されて壊れないよう、term を文字数の降順にソートしてから
  マッチングする。

`infra/lambda/conversation/index.js` の変更に伴い `cdk deploy` が
必要(「更新フロー」参照)。デプロイまで行わないと実際の会話には
反映されない。
